...
% TSEMO_VdV_piGP_Multiple_Runs.m
%
% Physics-informed TSEMO (piGP) parallel runner for the Van de Vusse PFR.
% Mirrors TSEMO_VdV_Multiple_Runs.m exactly, but calls
% TSEMO_V4_2_piGP_generic instead of TSEMO_V4_2_generic and saves the
% extra output theta_history (learned prior parameters per iteration).
%
% Required .m files on path / attached to workers:
%   TSEMO_V4_2_piGP_generic.m
%   piGP_prior_mean.m           (Van de Vusse version -- same filename as
%                                the SNAr version, keep in a separate dir)
%   NLikelihood_piGP.m          (Van de Vusse version, recentred MAP priors)
%   TrainingOfGP_piGP.m         (Van de Vusse version, recentred warm-start)
%   posterior_sample_piGP.m     (unchanged, fully generic)
%   f_van_de_vusse_pfr.m, f_van_de_vusse_kinetics.m, f_van_de_vusse_objective.m
%   TSEMO_options.m
%   + all original TSEMO package files (invChol, paretofront, nsga2,
%     Direct, hypervolume2D/3D mex files, etc.)
...
function run_single_piGP_optimization(run_num, batch_folder, maxeval, dataset_size)
    ...
    try
        ...
        %% Reactor + design specification (Van de Vusse, isothermal PFR)
        % PROVISIONAL -- kinetic parameter set and bounds not yet locked
        % (see piGP_Validation_Project_Context.md Section 5). Values
        % below reproduce the Chen/Kremling/Allgower (1995) set at a
        % provisional Tref=398.15K, matching f_van_de_vusse_pfr_validate.m.
        % Swap in final numbers once the epsilon-constraint sweep confirms
        % sensible bounds; everything downstream is agnostic to the change.

        R = 8.314; % J/mol/K
        k0_h = [1.287e12; 1.287e12; 9.043e9];   % [1/h; 1/h; L/(mol h)]
        Ea   = [9758.3; 9758.3; 8560] * R;       % J/mol
        Tref = 398.15;                           % K, PROVISIONAL
        k_ref_h = k0_h .* exp(-Ea / (R * Tref));
        k_ref   = k_ref_h / 3600;                % -> per-second basis

        cA_0 = 5.1; % mol/L
        rd   = 0.01; % relative noise, matches thesis in-silico convention

        % operating limits -- PROVISIONAL, revisit after epsilon-constraint
        tau_min = 0.1; %min
        tau_max = 5;   %min
        T_min   = 80;  %degC
        T_max   = 140; %degC
        ...
        f = @(x) f_van_de_vusse_objective(x, cA_0, k_ref, Ea, Tref, rd);
        ...
        % Uses the SAME f handle as the TSEMO loop below -- unlike the
        % original SNAr script, there's now only one place the
        % yield/selectivity formula lives (f_van_de_vusse_objective.m),
        % so the initial dataset and the mid-optimization re-evaluations
        ...
        %% Step 4: Start physics-informed TSEMO

        opt = TSEMO_options;
        opt.maxeval = maxeval;
        opt.NoOfBachSequential = 1;
        
        % TSEMO_V4_2_piGP_generic returns the extra output theta_history
        % vs. the plain TSEMO_V4_2_generic:
        %   theta_history [n_iter x 4 x 2]
        %     (:,:,1) = yield-of-B prior params [log_kref, Ea(J/mol), alpha, beta]
        %     (:,:,2) = selectivity prior params [log_kref, Ea(J/mol), alpha, beta]
        % Also note cA_0 is passed explicitly here (unlike the plain-TSEMO
        % call) -- piGP needs it directly for the physics prior mean, not
        % just inside the f closure.
        [Xpareto,Ypareto,X,Y,XparetoGP,YparetoGP,YparetoGPstd,hypf,theta_history] = ...
            TSEMO_V4_2_piGP_generic(f,X,Y,lb,ub,opt,cA_0);
        ...
        % Save workspace (theta_history added vs. the plain-TSEMO runner)
        save(['Workspace_' file_suffix '.mat'], ...
            'Xpareto', 'Ypareto', 'X', 'Y', 'XparetoGP', 'YparetoGP', 'YparetoGPstd', 'hypf', ...
            'theta_history', ...
            'lb', 'ub', 'cA_0', 'k_ref', 'Ea', 'Tref', 'rd', 'run_elapsed_time', 'maxeval', 'dataset_size');

        %% Step 5b: Save theta_history as CSV (one table per objective)
        ...
        T_yield = array2table([iter_col, theta_history(:,1,1), ...
                             theta_history(:,2,1)/1000, ...   % J/mol -> kJ/mol
                             theta_history(:,3,1), ...
                             theta_history(:,4,1)], ...
                            'VariableNames', col_names);
        writetable(T_yield, ['PriorParams_Yield_' file_suffix '.csv']);
        ...
        T_sel = array2table([iter_col, theta_history(:,1,2), ...
                             ...
                             theta_history(:,4,2)], ...
                            'VariableNames', col_names);
        writetable(T_sel, ['PriorParams_Selectivity_' file_suffix '.csv']);
        ...
        %% Step 6b: Prior parameter evolution diagnostic plot (piGP-specific)
        ...
        for p = 1:4
            ...
            if p == 1
                legend({'Yield of B', 'Selectivity'}, 'Location', 'best');
            end
        end
