function [OptGPhyp, theta_prior_out] = TrainingOfGP_piGP(Xnew, Ynew, OptGP, lb, ub, cA_0, obj_idx, theta_prior_init)
...
%   Van de Vusse version: cA_0 (was c1_0), default warm-start recentred
%   to Van de Vusse's scale (see below). GP/MAP machinery and theta_prior
%   BOUNDS are unchanged from the SNAr file -- already checked wide
%   enough to contain Van de Vusse's actual k_ref/Ea range without
%   modification (log_kref in [-14,5], Ea in [1000,1e5] J/mol; Van de
%   Vusse's true values sit comfortably inside both).
%
...
%    cA_0             scalar    initial concentration [mol/L]
...
%% Objective
obj_fun.f = @(hypVar) NLikelihood_piGP(hypVar, Xnew, Ynew, K_M, OptGP, lb, ub, cA_0, obj_idx);
...
if nargin < 8 || isempty(theta_prior_init)
    ...
    % Matches the log_kref/Ea centers used in NLikelihood_piGP.m's MAP
    % priors -- keep these two in sync if you retune either one.
    x0_prior = [-4.8; 75000; 1.0; 0.0];
else
